...
export default function ClientPage({ serverToken }: ClientPageProps) {
  const [mounted, setMounted] = useState(false);
  const [loginHovered, setLoginHovered] = useState(false);

  // High-Performance Optimization: Set the source directly from the server-rendered token.
  // The server-rendered HTML will already contain the correct, authenticated video src,
  // triggering the browser to fetch the video stream before React is even hydrated!
  const [videoUrl] = useState<string>(`/api/video?token=${serverToken}`);
